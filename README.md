...
*Exponential model: 2D Gaussian random field (top) with 100 random
sample locations (dots) and the resulting empirical semivariogram
vs. theoretical curve (bottom).*

------------------------------------------------------------------------

## What the app does

The app lets you simulate a Gaussian random field, draw a sample from
it, and see how well the empirical variogram recovers the true
covariance structure. It is designed to build intuition about:
...
deploy to [Hugging Face Spaces](https://huggingface.co) for free. All
deployment files are in `deploy/huggingface/` in this repository.
...
- Keep **Number of sample locations** at 100 or below for fast
  simulations on the hosted server
- The **Monte Carlo envelope** can take 1–2 minutes with 100 samples —
  warn students before they enable it, or reduce samples to 50 first
- Use **Number of simulations \> 1** with the animation slider to show
  students how the empirical variogram varies across realisations of the
  same process — this is one of the most effective teaching uses of the
  app
...
> for exploring variograms and Gaussian random fields* (v0.2.0).
